...
class Encoder {
 private:
  ...
  typedef typename _optraits::DataType _DataType; //_DataType is float32 or float16
  ...
  const float _fone; // _DataType
  const float _fzero; // _DataType

  const float _atten_scaler; // _DataType
 ...
 public:
  ...
  std::string check();
  void run_one_infer(int batch_size, int batch_seq_len, int infer_mode);
