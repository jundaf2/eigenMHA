...
class TransformerWeight {
 private:
  ...
  std::vector<const _DataType *> _p_d_src_emb_wei;  // size: 4, a vec of 4 pointers point to 4 arrays of FP16
  std::vector<const _DataType *> _p_d_trg_emb_wei;  // size: 4
  std::vector<const _DataType *> _p_d_enc_wei;      // size:   * enc_layer_num
 ...
 public:
  ...
  void print_model_config() {
    ...
    std::cout << "max step: " << _max_step << std::endl; // max seq len
