...
void Decoder<OpType_>::run_one_infer(int batch_size, int batch_seq_len) {
  ...
  _batch_max_decode_length =
      min(_tw._max_step, batch_seq_len + _tw._extra_decode_length) - 1;  // min(64,1+10)-1 = 10
...
bool Decoder<OpType_>::run_step() {
  ...
  if (_tw._sampling_method == "topk") { // this
    return sample();
  } else {
...
/**
Decoder self attention (FLASH-Attention Implementation)
*/
template <OperationType OpType_>
void Decoder<OpType_>::flash_self_attention() {
  /* ---step 0. --- */
}
...
bool Decoder<OpType_>::sample() {
  ...
  // std::cout << "ker_topk_sample_launcher with _cur_step=" << _cur_step << "  _max_step=" << _tw._max_step << "  _max_thread_per_block=" << _max_thread_per_block << std::endl;
...
#endif
  // for (int ii = 0; ii < 1; ii++) {// _batch_size
  //   print_vec(_p_d_alive_seq + ii * _tw._max_step, "Batch token ids", _cur_step + 2);
  // }
