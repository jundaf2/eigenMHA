...
void ker_norm_layer_launcher<__half>(int token_num, int hidden_size,
                                     ...
                                     int max_thread_per_block) {
  // max_thread_per_block -> hidden_size / 2
...
__global__ void ker_norm_layer_resual(T* input, T* output, const T* scale,
  ...
  uint block_start = blockIdx.x * hidden_size;
  uint start = block_start + threadIdx.x;   // start of this row of input (token embedding
  uint end = block_start + hidden_size;    // the end of this row of input (token embedding
  float val = 0.0;
  for (uint i = start; i < end; i += blockDim.x) {
    val += input[i];
  }
  ...
  // step 0. compute mean
  __shared__ float s_mean;
  float reduce_res = blockReduceSum<float>(val); // sum of a row of input
  if (threadIdx.x == 0) s_mean = reduce_res / float(hidden_size);  // mean of a row of input
  __syncthreads();
  ...
  __shared__ float s_var;
  reduce_res = blockReduceSum(val); // sum of the squared of a row of input
  if (threadIdx.x == 0)
    s_var = rsqrtf(reduce_res / float(hidden_size) + epsilon); // sqrt of variance
  __syncthreads();
...
__global__ void ker_norm_layer_resual<__half>(
  ...
  uint block_start = blockIdx.x * half_hidden_size; // half_hidden_size = 256
  uint start = block_start + threadIdx.x;
  uint end = blockIdx.x * half_hidden_size + half_hidden_size;
  half2* pinput = (half2*)input;
  ...
  float mean_dim = float(half_hidden_size) * 2.f; // the dim to get mean and var

  float val = 0.0;
  // step 0. compute mean
  for (uint i = start; i < end; i += blockDim.x) { // end = start + 256, blockDim=256
    float2 local_f2 = safe_half2_to_float2(pinput[i]);
    val += local_f2.x + local_f2.y;
  }
  __shared__ float s_mean;
  float reduce_res = blockReduceSum<float>(val); // sum of the row
  if (threadIdx.x == 0) s_mean = reduce_res / mean_dim; // mean of the row
  __syncthreads();
  ...
  __shared__ float s_var;
  reduce_res = blockReduceSum(val); // sum of the square of the row
  if (threadIdx.x == 0) s_var = rsqrtf(reduce_res / mean_dim + epsilon); // sqrt of the variance
  __syncthreads();
...
__global__ void ker_arrange_encself_qkv<__half>(
  ...
  int hidden_size = dim_per_head * head_num; // 256
  int batch_id = blockIdx.x / batch_seq_len; // which seq
  int token_id = blockIdx.x % batch_seq_len; // which token
  for (std::size_t i = threadIdx.x; i < hidden_size; i += blockDim.x) {
    int head_id = i / dim_per_head; // 0-256/32=0-8
    int dim_id = i % dim_per_head; // 0-32
    int qkv_offset = max_batch_dim * blockIdx.y;
    int target_id = targetid_4dim(batch_id, head_id, token_id, dim_id, head_num,
                                  batch_seq_len, dim_per_head);
...
__global__ void ker_arrange_atten_output(const T* ori_q, T* new_q,
                                         int beam_size, int dim_per_head,
                                         int head_num) {
  int hidden_size = dim_per_head * head_num; //512
  int batch_id = blockIdx.x / beam_size; // which seq
  // note, for encoder, beam_id is token_id; for decoder, beam_id is beam_id
  int beam_id = blockIdx.x % beam_size; // which token
  for (std::size_t i = threadIdx.x; i < hidden_size; i += blockDim.x) {
    int head_id = i / dim_per_head;
    int dim_id = i % dim_per_head;
    new_q[blockIdx.x * hidden_size + i] = ori_q[targetid_4dim(
        batch_id, head_id, beam_id, dim_id, head_num, beam_size, dim_per_head)]; // targetid_4dim: linearize 4-dim tensor index
  }
}
...
__global__ void ker_arrange_atten_output<__half>(const __half* ori_q,
  ...
  // note, for encoder, beam_id is token_id; for decoder, beam_id is beam_id
  int beam_id = blockIdx.x % beam_size;
  int half_hidden_size = dim_per_head * head_num; // 256
  for (std::size_t i = threadIdx.x; i < half_hidden_size; i += blockDim.x) {
    int head_id = i / dim_per_head; // 0-256/32 = 0-8
    int dim_id = i % dim_per_head;
...
__global__ void ker_topk_sample(const T* logits, const T* logit_bias,
                                ...
                                int eos_id) {
  int last_token_idx_in_batch = blockIdx.x * max_step + batch_seq_len - 1; // 0 + _cur_step 

  /* add EOS to end if last token is EOS */
  if (batch_seq_len > 1 && old_input_ids[last_token_idx_in_batch] == eos_id) { // batch_seq_len = 1
    if (threadIdx.x == 0) {
