...
Transformer::Transformer(const std::string weight_path,
    ...
    : LSModel({"source_ids"}, {"target_ids", "target_scores"}), // input and output with the likelyhood?
      stream_(nullptr),
      hd_(nullptr),
      decoder_(nullptr),
  ...
  */
  ...
  // CHECK_GPU_ERROR(
  //     cudaMalloc(&d_input_, _max_batch_size * tw_._max_step * sizeof(int32_t))); // max BS (128) x max seq len (64)
  ...
  CHECK_GPU_ERROR(cudaMalloc(
      &d_encoder_output_, _max_batch_size * tw_._max_step * tw_._hidden_size *
                              sizeof(optraits::DataType))); // the same as input -> max BS (64) x max seq len (64) x embedding size (512)
  CHECK_GPU_ERROR(
      cudaMalloc(&d_src_lang_id_, _max_batch_size * sizeof(int32_t))); // which language?
  CHECK_GPU_ERROR(
      cudaMalloc(&d_trg_lang_id_, _max_batch_size * sizeof(int32_t))); // which language?

  if (tw_._multilg_type < 3) {
    encoder_ = std::make_shared<Encoder<transformer_optytpe>>(
        _max_batch_size/*, d_input_*/, d_padding_mask_, d_encoder_output_, tw_, // d_input_ is p_d_token_id, but not used
...
void Transformer::Infer(int infer_mode) {
  int batch_size = input_shapes_[0][0], seq_len = input_shapes_[0][1];

  encoder_->run_one_infer(batch_size, seq_len, infer_mode);
