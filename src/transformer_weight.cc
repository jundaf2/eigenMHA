...
void TransformerWeight<OpType_>::proto_get_model_config(
  ...
  _max_step =
      transformer.trg_embedding().position_embedding_size() / _hidden_size; // _max_step is max seq len, _hidden_size is d_model (token embedding length and others)

  _inner_size =
      transformer.decoder_stack()[0].ffn_first_kernel_size() / _hidden_size; // _inner_size is the hidden layer size
  if (!only_decoder) {
    _src_vocab_size =
        transformer.src_embedding().token_embedding_size() / _hidden_size; // how many tokens in src 
  }
  _trg_vocab_size =
      transformer.trg_embedding().token_embedding_size() / _hidden_size; // how many tokens in dst
  ...
  _dim_per_head = _hidden_size / _head_num; // the dim of each head
  _weight_per_enc_layer = 12;
  _weight_per_dec_layer = 18;
  _beam_size = 1; //transformer.model_conf().beam_size();
  _extra_decode_length = 0; // transformer.model_conf().extra_decode_length();
  ...
  _sampling_method = "topk"; // transformer.model_conf().sampling_method();
  _topk = 1; //transformer.model_conf().topk();
  ...
  _multilg_type = 0; // transformer.model_conf().multilg_type();
}
...
std::string TransformerWeight<OpType_>::proto_parse_emb_wei(
  ...
  if (source == "src") {
    ...
    int count = 0;
    for (float e : value) raw_value.push_back(float2required(1.0f /*e*/));
...
std::string TransformerWeight<OpType_>::proto_parse_enc_wei(
  ...
  uint32_t count = 0;
  for (float e : value) {
    raw_value.push_back(float2required(0.1f/* *((count++)%5)  e*/));
  }
