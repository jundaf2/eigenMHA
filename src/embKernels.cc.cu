...
@param
token_emb: [vocab_size, hidden_dim] --> this is prefixed. indicated by integer token
pos_emb: [max_step, hidden_dim] --> this is also prefixed, since the length of the max seq is fixed. indicated by seq_idx
...
__global__ void ker_enc_emb(const T *token_emb, const T *pos_emb,
  ...
  decompose_3dim(idx, seq_len, hidden_dim, &batch_idx, &seq_idx, &dim_idx); // given seq_len and hidden_dim, which [batch_idx,seq_idx,dim_idx] does idx belong to
  int tokens_idx = batch_idx * seq_len + seq_idx; // which token am I?
  int token = tokens[tokens_idx]; // what token am I?
  float4 value;

  if (token == pad_id) { // if I am the padding token -> (0,0,0,0)
    if (dim_idx == 0) { // only use 1 thread to set the pad mask
    ...
    value.z = 0.f;
    value.w = 0.f;
  } else { // if I am other tokens
    if (dim_idx == 0) {
      pad_mask[tokens_idx] = 0; // this token is not the one should be padded
    }
    value = ((float4 *)token_emb)[token * hidden_dim + dim_idx]; // the embedding of the given token, read in parallel in consecutive threads. All the same token is equal.
    float4 pemb = ((float4 *)pos_emb)[seq_idx * hidden_dim + dim_idx]; // the given position embedding, read in parallel. Each one has its own depend on its position.
    value.x += pemb.x;
    value.y += pemb.y;
    value.z += pemb.z;
    value.w += pemb.w;
  }
  ((float4 *)output)[idx] = value; // write back to hbm consecutively
}
...
__global__ void ker_enc_emb<__half>(const __half *token_emb,
  ...
  int tokens_idx = batch_idx * seq_len + seq_idx; // seq_len=64
...
void launch_enc_emb(const T *token_emb, const T *pos_emb, const int *tokens,
  ...
  /* lang_id is not used if multilg_type=0 */
  if (hidden_dim % 4 != 0) {
    throw std::runtime_error("violate hidden_dim % 4 = 0");
  }
  hidden_dim >>= 2; // 4 float per thread
  ...
  if (multilg_type == 0) {
    ker_enc_emb<T><<<nblock, MAX_THREADS, 0, stream>>>(
        ...
        seq_len, hidden_dim); // hidden_dim is embedding size. hidden_dim/4 is 1 thread 4 element -> float4
...
void launch_enc_emb<__half>(const __half *token_emb, const __half *pos_emb,
  ...
  hidden_dim >>= 3; // div by 8,8 half per thread
  int nele = batch_size * seq_len * hidden_dim;
  int nblock = (nele + MAX_THREADS - 1) / MAX_THREADS;

  if (multilg_type == 0) { // this
